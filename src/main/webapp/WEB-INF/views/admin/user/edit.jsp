                    ...
                    <div id="profile">
                        ...
                        <div class="col-sm-12">
                            ...
                            <c:if test="${not empty model.id}">
                                ...
                                <img src="/static/admin/img/loading.gif" style="display: none; height: 100px" id="loading_image">
                            </c:if>
                            <c:if test="${empty model.id}">
                                <input type="button" class="btn btn-white btn-warning btn-bold"
...
<script>
    $("#btnAddOrUpdateUsers").click(function (event) {
        ...
        if (userId !== "") {
            roleCode = dataArray['roleCode'];
            if (roleCode !== '') {
                updateUser(dataArray, userId);
            } else {
                showAlert("Thông báo", "Yêu cầu cấp quyền cho người dùng", "info");
            }
        }
        else {
            ...
            if (userName !== '' && roleCode !== '') {
                ...
                showAlert("Thông báo", "Yêu cầu cấp đủ thông tin cho người dùng", "info");
            }
        }
    });
    ...
    function addUser(data) {
        $.ajax({
            ...
            success: function (res) {
                $('#loading_image').hide();
                showAlertThen("Thành công", "Đã thêm mới người dùng", function () {
                    window.location.href = "<c:url value='/admin/user-edit-" + res.id + "'/>";
                });
            },
            error: function (res) {
                showAlert("Lỗi", res, "error");
            }
        });
    }
    ...
    function updateUser(data, id) {
        $.ajax({
            ...
            data: JSON.stringify(data),
            success: function (res) {
                showAlertWithReload("Thành công", "Đã cập nhật người dùng");
            },
            error: function (res) {
                showAlert("Lỗi", res, "error");
            }
    ...
    function resetPassword(id) {
        $.ajax({
            ...
            success: function (res) {
                $('#loading_image').hide();
                showAlertThen("Thành công", "Đã khởi tạo lại mật khẩu", function () {
                    window.location.href = "<c:url value='/admin/user-edit-" + res.id + "'/>";
                });
            },
            error: function (res) {
